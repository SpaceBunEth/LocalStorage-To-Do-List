export default function App() {




return (
    <>
        {/* Header */}
        <div className="card text-center  h-50 position-absolute top-0 start-50 translate-middle-x w-75">
        <div className="card-header">
            <h3>To-Do List</h3>
        </div>


        <div className="card-header">
        <div className="row justify-content-center">
        <div class="input-group w-50">
        <input type="text" class="form-control" placeholder="Item Title..." aria-label="Recipient's username" aria-describedby="basic-addon2"/>
        <span class="input-group-text" id="basic-addon2">Create Item</span>
        </div>
        </div>
        
        {/* Description box for User */}
        {/* <div class="mb-3">
            <label for="exampleFormControlTextarea1" class="form-label">Description</label>
            <textarea class="form-control" id="exampleFormControlTextarea1" rows="3"></textarea>
        </div> */}

        </div>


        <div className="card-header">
            <ul className="nav nav-tabs card-header-tabs">
            <li className="nav-item">
                <a className="nav-link active" aria-current="true" href="#">Active</a>
            </li>
            <li className="nav-item">
                <a className="nav-link disabled" aria-current="true" href="#">Complete</a>
            </li>
            <li className="nav-item">
                <a className="nav-link disabled" aria-current="true" href="#">All</a>
            </li>

            </ul>
        </div>

        {/* Main */}
        <div className="card-body ">
        <h5 className="card-title">List of To-Do Items</h5>

        <div class="card">
        <div class="card-body p-0">
        <div className="row justify-content-around p-1">
            <div className='col-3 text-success'>
            <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" class="bi bi-check-square-fill " viewBox="0 0 16 16">
                <path d="M2 0a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2H2zm10.03 4.97a.75.75 0 0 1 .011 1.05l-3.992 4.99a.75.75 0 0 1-1.08.02L4.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093 3.473-4.425a.75.75 0 0 1 1.08-.022z"/>
            </svg>
              
            </div>
            <p className="col-3 m-0">Text about To-Do item:</p>

            <div class="dropdown col-1">
            <a class="text-secondary" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
            <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" class="bi bi-three-dots-vertical" viewBox="0 0 16 16">
            <path d="M9.5 13a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0zm0-5a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0zm0-5a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0z"/>
            </svg>
            </a>

            <ul class="dropdown-menu">
                <li><a class="dropdown-item" href="#">Description</a></li>
                <li><a class="dropdown-item" href="#">Edit</a></li>
                <li><a class="dropdown-item text-danger" href="#">Delete</a></li>
            </ul>
            </div>


            <div className='col-3 text-danger'>
                <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" class="bi bi-x-square-fill" viewBox="0 0 16 16">
                <path d="M2 0a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2H2zm3.354 4.646L8 7.293l2.646-2.647a.5.5 0 0 1 .708.708L8.707 8l2.647 2.646a.5.5 0 0 1-.708.708L8 8.707l-2.646 2.647a.5.5 0 0 1-.708-.708L7.293 8 4.646 5.354a.5.5 0 1 1 .708-.708z"></path>
                </svg>
              
            </div>
            </div>
        </div>
        </div>
        </div>

        {/* Footer */}
       <div className="container">
        <div className="card-footer text-muted row justify-content-around">
                <div className="col-2">
                    <p className='m-0 '>Total Items #</p>
                </div>
            <button type="button" class="btn btn-danger col-2">Clear All</button>
        </div>
        </div>
        
        </div>
    </>
);
}
